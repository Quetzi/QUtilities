...
public class SystemInfo {
    ...
    public static double getDimensionTPS(WorldServer worldServer) {
        double worldTickLength = mean(MinecraftServer.getServer().worldTickTimes.get(worldServer.provider.dimensionId)) * 1.0E-6D;
        return Math.min(1000.0 / worldTickLength, 20);
    }

    public static double getWorldTickTime(WorldServer worldServer) {
        return mean(MinecraftServer.getServer().worldTickTimes.get(worldServer.provider.dimensionId)) * 1.0E-6D;
    }
