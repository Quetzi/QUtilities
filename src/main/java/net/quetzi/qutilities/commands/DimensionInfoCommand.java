...
public class DimensionInfoCommand implements ICommand {
    ...
    private void showTPSSummary(ICommandSender sender) {
        ...
        for (WorldServer world : MinecraftServer.getServer().worldServers) {
            ...
            sender.addChatMessage(new ChatComponentText("[" + world.provider.dimensionId + "]"
                    + world.provider.getDimensionName() + ": "
                    + timeFormatter.format(worldTickLength) + "ms [" + worldTPS + "]"));
        }
    ...
    private void showTPSDetail(ICommandSender sender, int dimension) {
        ...
        sender.addChatMessage(new ChatComponentText("Players (" + world.playerEntities.size()
                + "): " + getPlayersForDimension(dimension)));
        sender.addChatMessage(new ChatComponentText("Item Entities: "
                + getItemEntityCount(world.loadedEntityList)));
        sender.addChatMessage(new ChatComponentText("Hostile Mobs: "
                + getHostileEntityCount(world.loadedEntityList)));
        sender.addChatMessage(new ChatComponentText("Passive Mobs: "
                + getPassiveEntityCount(world.loadedEntityList)));
        sender.addChatMessage(new ChatComponentText("Total Living Entities: "
                + getLivingEntityCount(world.loadedEntityList)));
        sender.addChatMessage(new ChatComponentText("Total Entities: "
                + world.loadedEntityList.size()));
        sender.addChatMessage(new ChatComponentText("Tile Entities: "
                + world.loadedTileEntityList.size()));
        sender.addChatMessage(new ChatComponentText("Loaded Chunks: "
                + this.world.getChunkProvider().getLoadedChunkCount()));
        sender.addChatMessage(new ChatComponentText("TPS: " + timeFormatter.format(worldTickTime)
                + "ms[" + worldTPS + "]"));
    }
    ...
    private String getPlayersForDimension(int dimension) {
        ArrayList<EntityPlayer> players = (ArrayList<EntityPlayer>) MinecraftServer.getServer()
                .worldServerForDimension(dimension).playerEntities;
        if (players.size() == 0) {
